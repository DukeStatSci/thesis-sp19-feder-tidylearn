...
in this STA 101 final project setting score higher on all three of the developed
metrics. Tidyverse projects overwhelmingly score higher in the upper levels of 
...
projects were lower, the resulting proportions were also much smaller. Still, though,
...
of new variables, and only differed marginally. This smaller difference
can be attributed to two factors. First, with options to create their own variables,
transforming existing ones may not seem as appealing to groups, and second, the
...
was the most popular one satisfied for both base R and tidyverse projects. Nearly
every tidyverse project performed a type of subgroup analysis, while more than 3/4
of base R projects did.
...
The popularity of this aspect can be explained by the copious amount of different
analyses student groups could use to perform a subgroup analysis. However, the 
...
also significantly more popular within Tidyverse projects, as more than 15 percent
of student projects in the tidyverse satisfied a score of a one for this covariate,
...
an obvious manner. However, student groups using the tidyverse may have been more
encouraged to utilize data subsets throughout their projects more often than those
in base R because other aspects of EDA have been done more frequently in tidyverse
projects, so those groups may have gained additional insights that led them to
using a data subset that base R project groups did not discover.

## Depth Metric

Although there is not as clear of a discrepancy in the depth metrics scores between
final projects that employed base R syntax compared to those using the tidyverse,
there is still a tangible difference in the depth metric distributions between
projects of the two syntaxes.
...
of base R projects scored a 0 or 1 in the depth metric. Projects of both syntaxes
mostly scored at least a one.
...
has a larger difference in proportions between base R and tidyverse student projects.
Compared to base R projects, where 57.3 percent boast a consistent theme, 71.5 percent
of all tidyverse projects maintain consistency theme-wise.
...
choose specific columns within a data that satisfy a certain criteria, where in 
base R, student groups commonly utilized square brackets to provide those same 
criteria.
...
Between projects using the two different syntaxes, the largest difference in
any of the three metrics occured within the multivariate visualization metric, where
...
visualizations, but projects using the tidyverse syntax fared far better in the 
metric in general, with 13 percent scoring a perfect two, while no projects in 
base R reached the same score.
...
ggplot(data = viz_counts, aes(x = multiviz, y = prop, fill = tidyverse, 
  ...
  geom_bar(stat = "identity", position = position_dodge(), width = .8) +
  scale_fill_viridis_d(name = "Syntax", labels = c("Base R", "Tidyverse"), begin = .37, end = .75) +
  labs(x = "Creativity Score", y = "Proportion of Projects", title = "Tidyverse Syntax Projects Score Higher on the Multivariate Visualization Metric on Average")
```
...
of the two syntaxes, there is still a clear divide.
...
Just one project coded in solely base R even displayed a visualization with at least
three variables, compared to the 30 tidyverse projects. Percentage-wise, too,
the tidyverse projects more significantly more likely to contain a multivariate
visualization,
...
These results can be attributed to the differences in base R's `plot()` function
relative to the tidyverse's `ggplot()`. Whereas it is arduous to specify plotting
aesthetics besides x- and y-variables in `plot()`, the `aes()` command embedded within
`ggplot()` provides an easy platform for tidyverse users to employ other aesthetics
besides just the x- and y-variables. For example, here is what the code would look
like to satisfy a score of a one for this covariate in the two syntaxes.
...
a one is not very high, more than half of tidyverse projects that formed a visualization
with at least three variables (16 out of 30) were able to sufficently interpret
the outputs.
...
compared to the tidyverse should not differ drastically. However, since there were
many more tidyverse projects that formed multivariate plots, there was bound to
be a difference in this study. In general, though, the plots should not deviate,
though some users may find comfort in viewing visualizations in the output provided
to their most commonly-used syntax.