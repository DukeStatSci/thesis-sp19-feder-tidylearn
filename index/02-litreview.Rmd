...
Students enrolled in tidyverse sections learn R through the use of the tidyverse's
tidy tools, which tidy and dissect data frames. But what is a tidy dataset?
It is one that lists each variable as a separate column and every observation as
...
Many popular base R functions require tidy data inputs, such as `lm()`, the base
R command for generating linear regressions. The tidyverse's primary author, 
Hadley Wickham, popularized the tidy framework based on his experience
tackling real-world problems and then his subsequent role as a teacher, creating
the suite of packages to provide a set of functions for analyzing and manipulating
datasets within this tidy framework. Wickham argues that most data transformation
operations are easiest to perform when each variable is listed as its column. The
consistent data structure allows users to start their analysis with the provided
dataset instead of spending time working out the logistics of the data. 
Additionally, there are only a few commands needed to tidy messy data frames, 
called tidy tools, so once those commands are mastered,
...
to another [@wickham2014tidy].”

As the tidyverse became more integrated into the general data science community, 
the mosaic package was created to help beginning coders develop advanced insights,
as explained by the package's authors. The authors believe that mosaic helps 
students to use programming in R as an asset from the start of their statistical
learning. Mosaic depends on a group of packages within the tidyverse suite and 
contains commands such as `mplot()`, which allows students to create interactive
plots in ggplot2. Mosaic is designed to be implemented alongside the tidyverse.
...
teaching beginning statistical programming, there has only been one study that 
...
R, but they were still beginners, as they had just completed a course covering 
basic R usage in both base R and the tidyverse.
...
Outside of the study conducted by Myint, Hadavand, Jager, & Leeke, additional 
arguments favoring base R or the tidyverse when teaching beginning statistics have
been made primarily on data science blogs. Dr. Jeff Leek and Dr. David Robinson
are two of the leaders on teaching in base R or the tidyverse, respectively, and
...
In response, Leek, who is a data scientist, biostatistician, and professor at
Johns Hopkins Bloomberg School of Public Health, argues  argues for using base R
...
By this time, the tidyverse had branched out from `ggplot2`, also incorporating
other packages such as `dplyr`. Given this development and his past teaching roles,
Robinson advocated for teaching the entire tidyverse to beginners. Robinson claims
